...
% run parameters
run_on_hopper = 1; % run from hopper
use_hopper = 1;
hopper_run = 2;
if run_on_hopper
    curr_dir=pwd;
...
function [Xs,Ys,animal_angles,animal_speeds]=loadTraj(angles_speeds, preloaded_XsYs, ...
    ...
    if angles_speeds
        if preloaded_XsYs ~= 1
            animal_angles = readmatrix('../../scripts/high_res_traj/anim_angles.csv');
            animal_speeds = readmatrix('../../scripts/high_res_traj/anim_speeds.csv');
        end
    else
        if run_on_hopper
            %hopper_path=(['/scratch/nsutton2/gc_sim/',int2str(hopper_run),'/spikes/highres_pos_x.csv']);
            hopper_path=(['/scratch/nsutton2/gc_sim/pe/spikes/highres_pos_x.csv']);
            Xs = readmatrix(hopper_path);
            %hopper_path=(['/scratch/nsutton2/gc_sim/',int2str(hopper_run),'/spikes/highres_pos_y.csv']);
            hopper_path=(['/scratch/nsutton2/gc_sim/pe/spikes/highres_pos_y.csv']);
    ...
    if use_spk_reader
        if run_on_hopper
            if plot_in_spikes
                ...
                hopper_path=(['/scratch/nsutton2/gc_sim/pe/results/spk_MEC_LII_Basket.dat']);
            else
                %hopper_path=(['/scratch/nsutton2/gc_sim/',int2str(hopper_run),'/results/spk_MEC_LII_Stellate.dat']);            
                hopper_path=(['/scratch/nsutton2/gc_sim/pe/results/spk_MEC_LII_Stellate.dat']);            
            end
        ...
        if run_on_hopper
            if plot_in_spikes
                ...
                hopper_path=(['/scratch/nsutton2/gc_sim/pe/spikes/in_spikes_recorded.csv']);
            else
                %hopper_path=(['/scratch/nsutton2/gc_sim/',int2str(hopper_run),'/spikes/spikes_recorded.csv']);
                hopper_path=(['/scratch/nsutton2/gc_sim/pe/spikes/spikes_recorded.csv']);
            end
