...
% set run params
t_start = 800;%800;%1;%800;%12400;%11400;%7000%12400;%8000;%10000;%5000; % start time
t_end = t_start+400;%400;%10000;%400;%600;%3000;%12800;%13000;%11000;%13000; % end time
nrns_tot = 1; % number of neurons to plot
i_start = 51;%100; % starting neuron index
plot_spikes = 1;
t_range = linspace(t_start,t_end,(t_end-t_start+1));
% resize peak params
rsz_peaks_active = 0;%1;%1; % toggle resizing of peaks
...
volt_nn = [55,103,55,123,102,117];%102%86%8
colors=[
[.224,.698,.898]; % stel
[.925,.608,.643]; % pyra
[.553,.157,.118]; % mp pyra
...
function [X,Y]=find_spikes(stel_nS, t_start, t_end, neuron_numbers, N, X, Y)
	for y=t_start:t_end
		for x2=1:N
			x=95+x2;%75+x2;%36+x2;%neuron_numbers(x2);
